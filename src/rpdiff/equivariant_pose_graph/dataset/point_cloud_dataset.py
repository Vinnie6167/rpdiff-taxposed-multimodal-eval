...
class PointCloudDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.distractor_anchor_aug:
            _, points_action, points_anchor1, points_anchor2, debug = get_random_rack_demo(
                        None, 
                        points_action, 
                        points_anchor,
        ...
        if(points_action.shape[1] > self.num_points):
            if np.random.random() < self.synthetic_occlusion and (self.occlusion_class == "all" or self.action_class == self.occlusion_class):
        ...
        if(points_anchor.shape[1] > self.num_points):
            if np.random.random() < self.synthetic_occlusion and (self.occlusion_class == "all" or self.anchor_class == self.occlusion_class):
                if self.ball_occlusion:
