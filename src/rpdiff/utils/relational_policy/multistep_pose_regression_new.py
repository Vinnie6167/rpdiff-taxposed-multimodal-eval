...
def multistep_regression_scene_combine_sc(
    ...
    assert child_pcd.ndim == 3

    rix1 = np.random.permutation(parent_pcd.shape[0])
    rix2 = np.random.permutation(child_pcd.shape[1])
    ...
    # build the size of the bounding box we will use
    child_mean = np.mean(child_pcd, axis=1, keepdims=True)
    child_pcd_scaled = child_pcd - child_mean
    child_pcd_scaled *= 1.25
    child_pcd_scaled = child_pcd_scaled + child_mean
    try:
        child_bb = trimesh.PointCloud(child_pcd_scaled[0]).bounding_box.to_mesh()
    ...
    M = k_val
    assert M == child_pcd.shape[0] # assert batch size is correct
    N_crop = 2048
    ...
    # child_pcd is already a batch
    child_pcd_t = torch.from_numpy(child_pcd).float().cuda()
    child_pcd_ds = fps_downsample(child_pcd_t, N_crop)  # batch x N_crop x 3
    ...
    # make copies of child point cloud and child pcd mean (points + batch) + initial translation points (points) 
    child_pcd_batch = child_pcd_ds
    child_mean_batch = child_pcd_t.mean(1).unsqueeze(1)
    world_trans_batch = world_pts_t.unsqueeze(1)
    ...
    if export_viz:
        ...
        for ii in export_all_idx:
            ...
            tf_cent = np.eye(4); tf_cent[:-1, -1] = -1.0 * np.mean(child_pcd_original, axis=(0,1))
    ...
    with torch.no_grad():
        for ii in range(n_iters):
            ...
            if export_viz:
                ...
                # should change this since child_pcd_ds is now batch * n * 3 instead of 1 * n * 3
    ...
    if util.exists_and_true(kwargs, 'compute_coverage_scores'):
        ...
        success_topk_idx = torch.topk(success_out_all, k = M).indices
    else:
        success_topk_idx = torch.topk(success_out_all, k = M).indices
    ...
    output_idx = success_topk_idx.detach().cpu().numpy()[0]
    print(f'Top value: {success_top_val.item():.4f}')
    ...
    tmat_full_list = []
    ii_bucket = 0
    for ii, k_idx in enumerate(success_topk_idx):
        ...
        child_pcd_ds_np = child_pcd_ds[k_idx].detach().cpu().numpy()
        assert child_pcd_ds_np.shape == (N_crop, 3), f"shape is wrong, shape is {child_pcd_ds_np.shape}"
        child_pcd_cent = child_pcd_ds_np - np.mean(child_pcd_ds_np, axis=0)        
    ...
    child_pcd_ds_np = child_pcd_ds[output_idx].detach().cpu().numpy()
    ...
    if util.exists_and_true(kwargs, 'compute_coverage_scores'):
        ...
        for i, (tmat, k_idx) in enumerate(zip(tmat_full_list, success_topk_idx)):
            
            tf_cent = np.eye(4); tf_cent[:-1, -1] = -1.0 * np.mean(child_pcd_original[k_idx], axis=0)
            tmat_w_guess = np.matmul(tmat, np.matmul(tf_cent, kwargs['export_viz_relative_trans_guess'][i]))
