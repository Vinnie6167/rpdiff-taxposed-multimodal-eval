...
def main(args: config_util.AttrDict) -> None:
    ...
    cfg = compose(config_name="train_bookshelf_residual_vinnie.yaml")
    # cfg = compose(config_name="can_on_cabinet_residual_vinnie.yaml")
    ...
    for iteration in range(exp_args.start_iteration, exp_args.num_iterations):
        ...
        print(f"{exp_args.eval.taxposed_input=}")
        if exp_args.eval.taxposed_input:
            ...
            # Setup success model input
            parent_pcd_mean = np.mean(parent_pcd, axis=0)
            sm_input = {
                'parent_start_pcd': torch.tensor((parent_pcd - parent_pcd_mean)[None, :, :]).float().cuda(),
                'parent_start_pcd_mean': torch.tensor(parent_pcd_mean[None, :]).float().cuda(),
            }


            # Sample several transformations, score them, take the best one
            num_samples = 50
            samples = []
            best_score = -1
            for i in range(num_samples):
                ...
                samples.append(pred_T_action_mat)

                # apply transform
                child_pcd_guess = util.transform_pcd(child_pcd, pred_T_action_mat)

                child_pcd_guess_mean = np.mean(child_pcd, axis=0)
                # child_pcd_guess_mean = np.mean(child_pcd_guess, axis=0)
                sm_input.update({
                    'child_start_pcd': torch.tensor((child_pcd_guess - child_pcd_guess_mean)[None, :, :]).float().cuda(),
                    'child_start_pcd_mean': torch.tensor(child_pcd_guess_mean[None, :]).float().cuda(),
                })

                # Visualize point clouds to see if we are transforming incorrectly or scoring incorrectly
                # import open3d as o3d

                # cpcd_geom_start = o3d.geometry.PointCloud()
                # cpcd_geom_start.points = o3d.utility.Vector3dVector(child_pcd)
                # cpcd_geom_start.paint_uniform_color(np.array([[0], [0], [1]]))
                # cpcd_geom_end = o3d.geometry.PointCloud()
                # cpcd_geom_end.points = o3d.utility.Vector3dVector(child_pcd_guess)
                # cpcd_geom_end.paint_uniform_color(np.array([[0], [1], [0]]))
                # ppcd_geom = o3d.geometry.PointCloud()
                # ppcd_geom.points = o3d.utility.Vector3dVector(parent_pcd)
                # ppcd_geom.paint_uniform_color(np.array([[1], [0], [0]]))
                # o3d.visualization.draw_geometries([ppcd_geom, cpcd_geom_end, cpcd_geom_start])

                score = success_model(sm_input)['success']

                print(f"sample {i} got score of {score}")

                if score > best_score:
                    best = pred_T_action_mat
                    best_score = score

            relative_trans_guess = best
        ...
        # Loop through each transform and compute child_pcd_guess

        # Score each transform

        # Select the top transform
