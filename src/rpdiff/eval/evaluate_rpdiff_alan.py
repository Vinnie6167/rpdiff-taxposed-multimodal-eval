...
from rpdiff.utils.relational_policy.multistep_pose_regression_new import policy_inference_methods_dict
...
def main(args: config_util.AttrDict) -> None:
    ...
    cfg = compose(config_name="train_mug_residual")
    ...
    for iteration in range(exp_args.start_iteration, exp_args.num_iterations):
        ...
        if child_pcd.shape[0] < 300:
            ...
            mc_vis['scene/child_pcd_orig'].delete()
        ...
        if exp_args.eval.taxposed_input:
            ...
            repeat = exp_args.eval.init_k_val
            ...
            num_split = repeat // 8
            pchilds = torch.split(points_child, num_split)
            pparents = torch.split(points_parent, num_split)

            mat_T = []
            for i, (pchild, pparent) in enumerate(zip(pchilds,pparents)):
                ans = place_model.get_transform(pchild, pparent)

                pred_T_action_init = ans["pred_T_action"]
                pred_T_action_mat = pred_T_action_init.get_matrix().transpose(1,2).detach().cpu().numpy() # k * 4 * 4
                # pred_T_action_mat = pred_T_action_init.get_matrix().detach().cpu().numpy() # k * 4 * 4
                
                mat_T.append(pred_T_action_mat)

            relative_trans_guess = np.concatenate(mat_T)
            ...
            relative_trans_guess = relative_trans_guess[None].repeat(exp_args.eval.init_k_val, axis=0)

        time.sleep(0.3)

        child_pcd_guess = np.array([util.transform_pcd(child_pcd, guess) for guess in relative_trans_guess])
        ...
        if skip_refinement:
            relative_trans_pred, child_idx = np.eye(4), 0
        else:
            # refine
            relative_trans_pred, child_idx = infer_relation_policy(
                mc_vis, 
        ...
        print(relative_trans_pred, child_idx)
        child_pcd_guess = child_pcd_guess[child_idx]
        relative_trans_guess = relative_trans_guess[child_idx]
        transformed_child1 = util.transform_pcd(child_pcd_guess, relative_trans_pred)

        with recorder.meshcat_scene_lock:
            util.meshcat_pcd_show(mc_vis, child_pcd, color=[255, 0, 0], name='scene/child_pcd_orig')
